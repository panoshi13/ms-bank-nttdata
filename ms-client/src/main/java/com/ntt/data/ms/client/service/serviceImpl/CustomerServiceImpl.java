...
public class CustomerServiceImpl implements CustomerService {
        ...
        return creditApiClient.get()
            ...
            .onStatus(httpStatus -> httpStatus.value() == 400,
                clientResponse -> Mono.error(
                    new RuntimeException("Error 400 en consulta de movimientos de tarjeta")))
    ...
    public Mono<BankAccountDTO> fetchBankAccount(String productId) {
        return bankAccountApiClient.get()
            ...
            .onStatus(httpStatus -> httpStatus.value() == 400,
                clientResponse -> Mono.error(
                    new CustomException("Error 400 en consulta de movimientos de tarjeta")))
    ...
    public Flux<BankAccountDTO> fetchBankAccountByClientId(String clientId) {
        return bankAccountApiClient.get()
            ...
            .onStatus(httpStatus -> httpStatus.value() == 400,
                clientResponse -> Mono.error(
                    new CustomException("Error 400 en consulta de la cuenta bancaria por cliente")))
    ...
    public Flux<CreditDTO> fetchCreditByClientId(String clientId) {
        return creditApiClient.get()
            ...
            .onStatus(httpStatus -> httpStatus.value() == 400,
                clientResponse -> Mono.error(
                    new CustomException("Error 400 en consulta del credito por cliente")))
    ...
    @Override
    public Mono<CustomerProductBalanceResponse> getBalanceAvailable(String clientId,
                                                                    String productId) {
        return customerRepository.findById(clientId)
            .switchIfEmpty(Mono.error(new RuntimeException("Cliente no encontrado")))
    ...
    public Mono<MonthlyBalanceReportResponse> getMonthlyBalanceReport(String clientId) {
        ...
        return Mono.zip(
            ...
            .flatMap(tuple -> {
                ...
                List<MonthlyBalanceReportResponseAverageBalancesBankAccounts>
                    balancesBankAccountsProducts = bankAccounts.stream()
                    .map(account -> {
                        MonthlyBalanceReportResponseAverageBalancesBankAccounts
                            balancesBankAccounts =
                            new MonthlyBalanceReportResponseAverageBalancesBankAccounts();
                        LocalDate today = LocalDate.now();
                        int dayOfMonth = today.getDayOfMonth();
                        balancesBankAccounts.setAccountId(account.getId());
                        var balanceAverage = account.getBalance() / dayOfMonth;
                        balancesBankAccounts.setAverageDailyBalance(
                            aroundTwoDecimal(balanceAverage));
                        return balancesBankAccounts;
                    })
                    .collect(Collectors.toList());

                // Calcular saldos promedio diarios
                List<MonthlyBalanceReportResponseAverageBalancesCreditProducts>
                    balancesCreditProducts = credits.stream()
                    .map(credit -> {
                        MonthlyBalanceReportResponseAverageBalancesCreditProducts creditProducts =
                            new MonthlyBalanceReportResponseAverageBalancesCreditProducts();
                        LocalDate today = LocalDate.now();
                        int dayOfMonth = today.getDayOfMonth();
                ...
                MonthlyBalanceReportResponseAverageBalances averageBalances =
                    new MonthlyBalanceReportResponseAverageBalances();
    ...
    @Override
    public Mono<ConsolidatedSummaryResponse> getConsolidatedSummary(String clientId) {
        Mono<Customer> customerMono = customerRepository.findById(clientId);
        Flux<BankAccountDTO> bankAccountDTOFlux = fetchBankAccountByClientId(clientId)
            .onErrorResume(throwable -> {
                log.error("Error al obtener saldo de cuenta bancaria: {}", throwable.getMessage());
                return Flux.empty();
        ...
        return Mono.zip(
            ...
            .flatMap(tuple -> {
                ...
                List<BankAccountDTO> bankAccounts = tuple.getT2();
                List<CreditDTO> credits = tuple.getT3();

                ConsolidatedSummaryResponse summaryResponse = new ConsolidatedSummaryResponse();
                summaryResponse.setClientId(String.valueOf(customer.getId()));
                summaryResponse.setName(customer.getName());

                List<ConsolidatedSummaryResponseProducts> products = bankAccounts.stream()
                    .map(bankAccountDTO -> {
                        ConsolidatedSummaryResponseProducts responseProducts =
                            new ConsolidatedSummaryResponseProducts();
                        responseProducts.setStatus(bankAccountDTO.getStatus());
                        responseProducts.setType(bankAccountDTO.getType().name());
                        responseProducts.setId(bankAccountDTO.getId());
                        return responseProducts;
                    }).collect(Collectors.toList());

                List<ConsolidatedSummaryResponseProducts> creditProducts = credits.stream()
                    .map(creditDTO -> {
                        ConsolidatedSummaryResponseProducts responseProducts =
                            new ConsolidatedSummaryResponseProducts();
                        responseProducts.setStatus(creditDTO.getStatus());
                        responseProducts.setType(creditDTO.getType().name());
                        responseProducts.setId(creditDTO.getId());
                        return responseProducts;
                    }).toList();

                products.addAll(creditProducts);
                summaryResponse.setProducts(products);
    ...
    public Mono<CustomerProductBalanceResponse> getCustomerProductBalanceResponse(
        String productId,
        Customer customer) {
        ...
        Mono<CreditDTO> creditDTOMono = fetchCredit(productId)
            .onErrorResume(throwable -> {
                log.error("Error al obtener saldo de cuenta de crédito: {}",
    ...
    private static CustomerProductBalanceResponse getBalanceResponse(Customer customer,
                                                                     Tuple2<BankAccountDTO,
                                                                         CreditDTO> tuple) {
        ...
        balanceResponse.typeCard(bankAccountDTO.getType() == null ? creditDTO.getType().name() :
            bankAccountDTO.getType().name());
        balanceResponse.availableBalance(
            bankAccountDTO.getType() == null ? creditDTO.getAvailableBalance() :
                bankAccountDTO.getBalance());
        return balanceResponse;
    }

    public Mono<CustomerProductMovementsResponse> getProductDTOMono(String productId,
                                                                    Customer customer) {
        Mono<BankAccountDTO> bankAccountDTOMono = fetchBankAccount(productId)
            .onErrorResume(throwable -> {
                log.error("Error al obtener movimientos de cuenta bancaria: {}",
                    throwable.getMessage());
                return Mono.empty();
            });
        Mono<CreditDTO> creditDTOMono = fetchCredit(productId)
            .onErrorResume(throwable -> {
                log.error("Error al obtener movimientos de cuenta de crédito: {}",
                    throwable.getMessage());
                return Mono.empty();
            });
        ...
        return Mono.zip(
            ...
            .flatMap(hasElements -> {
                ...
                if (!hasBankAccount && !hasCreditAccount) {
                    return Mono.error(
                        new CustomException("No se pudieron obtener los movimientos"));
                }
                return Mono.zip(
                        bankAccountDTOMono.defaultIfEmpty(new BankAccountDTO()),
                        creditDTOMono.defaultIfEmpty(new CreditDTO())
    ...
    public CustomerProductMovementsResponse getMovementsResponse(Customer customer,
                                                                 Tuple2<BankAccountDTO,
                                                                     CreditDTO> tuple) {
        BankAccountDTO bankAccountDTO = tuple.getT1();
        CreditDTO creditDTO = tuple.getT2();
        ...
        movementsResponse.setMovementsBankAccount(
            clientMapper.movementsBankAccounts(bankAccountDTO.getMovements()));
